...
export async function GET(request: NextRequest) {
  ...
  async function processSendLog(sendLog: [ethers.EventLog, CHAIN]) {
    ...
    let [srcPortAddress, srcChannelId, packet, sequence, timeout, fee] =
      sendEvent.args;
    ...
    const channel = openChannels.find(
      (channel) =>
  ...
  async function processAckLog(ackLog: [ethers.EventLog, CHAIN]) {
    ...
    const key = `${srcPortAddress}-${ethers.decodeBytes32String(
      srcChannelId
    )}-${sequence}`;
  ...
  let destChainProviders: Record<CHAIN, CachingJsonRpcProvider> = {} as Record<
    CHAIN,
    CachingJsonRpcProvider
  >;
  ...
  await Promise.allSettled(
    Array.from(unprocessedPacketKeys).map(processPacket)
  );
  ...
  const promises = recvPacketLogs.map(async (recvPacketLog) => {
    ...
    if (packets[key]) {
      const recvBlock = await destChainProviders[destChain].getBlock(
        recvPacketEvent.blockNumber
      );
