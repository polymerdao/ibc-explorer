...
export async function calcMetrics(
  ...
  return await Promise.all(
    Object.keys(CHAIN_CONFIGS).map(async (chain) => {
      ...
      console.log(
        `Block Number for ${chain} for Start Date: ${blockStartNumber}`
      );
      ...
      try {
        response = await fetch(
          `${origin}/api/metrics?from=${blockStartNumber}&to=${blockEndNumber}&chain=${chain}`
        );
      } catch (error) {
        console.error(
          `Failed to fetch metrics for chain ${chain} origin ${origin}:`,
          error
        );
